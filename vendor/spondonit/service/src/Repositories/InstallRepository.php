...
class InstallRepository
{
    ...
    public function validateLicense($params)
    {
        ...
        Storage::put('.temp_app_installed', 'adadsvfasd');
        Storage::put('.access_code', '123466252');
        Storage::put('.account_email', 'das@gmail.com');
      ...
      //  $response = curlIt($url);
        ...
        if (true) {
            ...
            try {
                ...
                $s->checksum = gv('manually-inserted', 'checksum');
                $r = $s->save();
            ...
            if (request()->wantsJson()){
                throw ValidationException::withMessages(['message' => gv('not installed', 'message', 'Something is not right')]);
            }
            Toastr::error(gv('not installed', 'message', 'Something is not right'));
            return false;
        }
    }
